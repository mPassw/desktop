...
use super::{
    srp_group::{hex_to_biguint, N_HEX},
...
pub async fn generate_salt() -> Result<String, String> {
    let salt = utils::generate_random_bytes(16).map_err(|e| e.to_string())?;

    Ok(hex::encode(salt))
}

#[tauri::command]
pub async fn generate_verifier(
    username: String,
    password: String,
    salt: String,
...
#[tauri::command]
pub async fn generate_srp_credentials() -> Result<SrpCredentials, String> {
    let n = hex_to_biguint(N_HEX);
    let g = BigUint::from(2u8);

    let random_bytes = utils::generate_random_bytes(32).map_err(|e| e.to_string())?;
...
#[tauri::command]
pub async fn calculate_srp_proof(
    private_key: String,
    username: String,
    password: String,
    salt: String,
