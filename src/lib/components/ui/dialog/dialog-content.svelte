<script lang="ts">
	...
	import { loadersState } from "$lib/modules/loaders/loaders.state.svelte.js";
	import DialogLoader from "$lib/components/layout/loaders/dialogLoader.svelte";
	...
	>
		{#if loadersState.isDialogLoaderVisible}
			<DialogLoader
				width={192}
				height={192}
				status={loadersState.dialogLoaderStatus}
			/>
		{/if}
		{@render children?.()}
